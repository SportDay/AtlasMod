package me.xtrm.Atlas.utils.tpaura;
...
public class TPCore {
	...
	public static int getXDiff(Entity e) {
		return (int)Math.round(Wrapper.mc.thePlayer.posX - e.posX);
	}
	
	public static int getYDiff(Entity e) {
		return (int)Math.round(Wrapper.mc.thePlayer.posY - e.posY);
	}
	
	public static int getZDiff(Entity e) {
		return (int)Math.round(Wrapper.mc.thePlayer.posZ - e.posZ);
	}
	...
	public static TPPath getTPPathToEntity(Entity e) {
		...
		int yTPTimes = (int)Math.round(getYDiff(e) / tpBuffer); // kinda useless
